drop sequence if exists seqid_serial;

CREATE SEQUENCE seqid_serial start 1 increment 1 maxvalue 2147483647 minvalue 1  cache 1 ;

drop sequence if exists hit_id_serial;

CREATE SEQUENCE hit_id_serial start 1 increment 1 maxvalue 2147483647 minvalue 1  cache 1 ;
...
create table seq_data (
       ...
       length            int4,
       create_time       timestamp with time zone,
       seq_type          text);  -- We do want OID's for this table

create index seq_data_seqid on seq_data using btree (seqid text_ops);
create index seq_data_checksum on seq_data using btree (checksum bpchar_ops);
...
create index seq_pieces_seqid on seq_pieces using btree (seqid text_ops, segnum int4_ops);
...
create table raw_seqs (
       ...
       concatid   text);  -- OID's wanted.

create index raw_seqs_name on raw_seqs using btree (name text_ops);
create index raw_seqs_seqid on raw_seqs using btree (seqid text_ops);
create index raw_seqs_library on raw_seqs using btree (library text_ops);
create index raw_seqs_id on raw_seqs using btree (repository, library, name_type, name);
create index raw_seqs_concatid on raw_seqs using btree (concatid);
...
create index duplicate_raw_seqs_name on duplicate_raw_seqs using btree (name text_ops);
create index duplicate_raw_seqs_seqid on duplicate_raw_seqs using btree (seqid text_ops);
...
create index filtered_seqs_name on filtered_seqs using btree (name text_ops);
create index filtered_seqs_seqid on filtered_seqs using btree (seqid text_ops);
...
create index duplicate_filtered_seqs_name on duplicate_filtered_seqs using btree (name text_ops);
create index duplicate_filtered_seqs_seqid on duplicate_filtered_seqs using btree (seqid text_ops);
...
create index hits_seqid on hits using btree (seqid text_ops);
create index hits_id on hits using btree (hit_id text_ops);
create index hits_db_all on hits using btree (seqid text_ops,
					      common_db_name text_ops,
					      method text_ops);
create index hits_db_key on hits using btree (common_db_name text_ops,
					      key text_ops);
...
create index hsps_id on hsps using btree (hit_id text_ops);

grant select on hsps to public;
...
create index annotation_runs_seqid on annotation_runs using btree (seqid text_ops);
...
drop sequence if exists annotator_run_count;

CREATE SEQUENCE annotator_run_count start 1 increment 1 maxvalue 2147483647 minvalue 1  cache 1 ;
...
create index annotator_history_db
    on annotator_history
 using btree
       (library_number, common_db_name, method);
...
create index genscan_seqid on genscan using btree (seqid text_ops);
...
create index pred_pep_n_seqid on pred_pep using btree (n_seqid text_ops);
...
drop sequence if exists cluster_serial;

create sequence cluster_serial;
...
insert into contig_run (number, program, create_time, completed)
       values(0, 'placeholder', 'now', 'f');

grant all on contig_run to public;
...
create index curation_seqid on curation using btree (seqid text_ops);
create index curation_user on curation using btree (username text_ops);
